...
<script type="application/javascript">
    $(function(){
        ...
        function loadTree(){
            $.ajax({
                url:"/sys/dept/findTree.json",
                success:function(data){
                    console.log(data);
                    if(data.ret){
                        var rendered = Mustache.render(deptListTemplate,{deptList:data.data});
                        $("#deptList").html(rendered);
                        whileLoadSonTree(data.data);

                    }else{
                        showMessage("展示部门信息失败",data.msg,true);
                    }
                }
            })
        }
        ...
        /**
         * 循环加载子节点 这里只会加载子节点,当前节点不会加载
         * @param depts
         */
        function whileLoadSonTree(depts){
            if(depts != null && depts.length > 0){
                $(depts).each(function(i,dept){
                    if (dept.deptList.length > 0) {
                        ...
                        whileLoadSonTree(dept.deptList);
                    }
                })
            }
        }
